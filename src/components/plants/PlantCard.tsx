...
export function PlantCard({ plant, onClick, onEdit, onDelete }: PlantCardProps) {
  const { t } = useTranslation(['plants', 'accessibility']);
      ...
      <div className="p-4">
        <div className="flex justify-between items-start mb-2">
          ...
          <div className="flex space-x-1">
            <button
              ...
              title={t('accessibility:editPlant')}
            >
            ...
            <button
              ...
              title={t('accessibility:deletePlant')}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="space-y-2">
          {overdueTasks.length > 0 && (
            <div className="flex items-center text-xs text-red-600 bg-red-50 px-2 py-1 rounded">
              ...
              {overdueTasks.length} {overdueTasks.length === 1 ? t('plants:tasks.overdue') : t('plants:tasks.overduePlural')}
            </div>
          )}
          ...
          {upcomingTasks.length > 0 && (
            <div className="flex items-center text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded">
              ...
              {t('plants:tasks.next')} {formatDistanceToNow(upcomingTasks[0].dueDate, { addSuffix: true })}
            </div>
          )}
          
          {plant.careTasks.length === 0 && (
            <div className="text-xs text-gray-500">
              {t('plants:tasks.noCareTasks')}
            </div>
          )}
        </div>
        ...
        <div className="mt-3 pt-3 border-t border-gray-100">
          <p className="text-xs text-gray-500">
            {t('plants:tasks.updated')} {formatDistanceToNow(plant.updatedAt, { addSuffix: true })}
          </p>
        </div>
      </div>
    </div>
